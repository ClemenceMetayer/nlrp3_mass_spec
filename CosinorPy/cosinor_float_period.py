...
def get_best_fits(df, df_results):
    df_best = pd.DataFrame(columns = df_results.columns, dtype=float)
    names = np.unique(df_results.test)
    
    for name in names:
        df_best_name = pd.DataFrame(columns = df_results.columns, dtype=float)
        n_points = df[df.test.str.startswith(name)].x.shape[0] 
        df_test_models = df_results[df_results.test == name]
        
        for new_row in df_test_models.iterrows():
            # Testing if the new_row is a better fit than the Mesor for each period
            mesor = new_row[1].mesor
            data_mesor = df[df.test == name].y - mesor
            data_mesor = data_mesor**2
            
            RSS_reduced = sum(data_mesor)
            RSS_full = new_row[1].RSS
                
            DF_reduced = n_points - 1
            DF_full = n_points - 3
                
            if n_points > 3 and compare_models(RSS_reduced, RSS_full, DF_reduced, DF_full) < 0.05:
                best_row = new_row[1]
            else :
                best_row = pd.Series (index = new_row[1].index)
                best_row.test = name
                best_row.period = 0
                best_row.n_components = 0
                best_row.RSS = RSS_reduced
                best_row.mesor = mesor
                
            # Store the results of the best fit between each period and mesor 
            df_best_name = df_best_name.append(best_row, ignore_index=True)
            
            # Minimum RSS of the df_best_name :
            # Minimum of the significative period
            M = df_best_name[df_best_name.period != 0].RSS.min()
            # If there is no significative period, mininimum of the mesor model
            if np.isnan(M) :
                M = df_best_name.RSS.min()

            best_period = df_best_name[df_best_name.RSS == M]
            
            # If there is more than one case with the same RSS, we peak the first model 
            if len(best_period) != 1 :
                best_period = best_period.sort_values(by=['period'])
                best_period = best_period.iloc[0,:]
                
            best_period.n_points = n_points
            
        df_best = df_best.append(best_period, ignore_index = True)
...
def get_best_models(df, df_models, n_components = [1,2,3], lin_comp = False, criterium='p', reverse = True):
    ...
    for test in names:  
        n_points = df[df.test == test].x.shape[0] # Number of points for a prot/cond 
        df_test_models = df_models[df_models.test == test] # Store the models to test for this protein
        df_test_models = df_test_models.sort_values(by=['n_components'])
        i = 0
        # Iteration on the different test
        for new_row in df_test_models.iterrows():   
            if i == 0:
            # if the new_row is the first/the only one then its the best
                best_row = new_row
                i = 1
            else:
            # if there are multiple rows in competition 
                RSS_reduced = best_row[1].RSS # RSS of the first row
                RSS_full = new_row[1].RSS # RSS of the row to test
                ...
                if DF_reduced != DF_full and compare_models(RSS_reduced, RSS_full, DF_reduced, DF_full) < 0.05 :
                    best_row = new_row
                
                # Condition added for the case of comparing a model with different period but same n_components
                if DF_reduced == DF_full and new_row[1].RSS < best_row[1].RSS :
                    best_row = new_row
