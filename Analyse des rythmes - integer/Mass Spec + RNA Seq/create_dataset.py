...
# For every protein and for every time point, we have 4 control replicates and 4 sinlrp3 replicates 
ms = pd.read_csv('data/20221107133514_ALHuber_log10', sep='\t')
# Keep only the rows with the data of interest
ms = ms.drop(ms.index[range(10570,31724)])

ms.index = ms.loc[:, 'SUM_INT.txt'] # Line names
...
NLRP3_cols = [col for col in ms.columns if 'NLRP3' in col] # Column names siNRLP3
CTRL_cols = [col for col in ms.columns if 'NLRP3' not in col] # Column names siCTRL
nprots =  len(ms)
...
cts = [4, 8, 12, 16, 20, 24, 28, 32, 36] #Circadian time
# Creation of a list with 9 list in it corresponding to the name of colummns by replicates at a certain circadian time.
...
for idx_cond, cond in enumerate(conds):
    # Iteration on the number of proteins 
     for n in range(nprots):
          ...
          data[cond][ms.index[n]] = prot 
# New key in the data dictionnary which store the circadian times
data["CTs"] = cts
# New key in the data dictionnary which store the names of ms columns
data["cols"] = ms.columns
...
nreps = 4
# Creation of ncols (9 circadian times * 4 replicates) and names which store all the names of protein
ncols, names = len(data["CTs"]) * nreps, list(data["ctrl"].keys())
# CCreation of the cols list which store the names ZT_i_j+1 for every circadian time and every replicate
